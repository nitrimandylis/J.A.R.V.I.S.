---
name: start
description: Session briefing for Nick's JARVIS assistant. Use when starting a session, or when Nick says "brief me", "start", "what's going on", or asks for a status update on assignments and projects.
---
...
9. **Gmail** (opt-in only) — check if Nick has opted in to Gmail triage during session start. If yes, run `bun run tools/gmail-triage.ts`. If not, skip silently.
